...
export default function ContextScreen() {
  ...
  const [previousSelection, setPreviousSelection] = useState(selectedTurn);
  if (selectedTurn !== previousSelection) { setPreviousSelection(selectedTurn); if (selectedTurn) { setTab(0); setSearch(''); } }
  useEffect(() => { if (selectedTurn) { const timer = setTimeout(() => scroll.current?.scrollTo({ y: positions.current[selectedTurn] ?? 0, animated: true }), 350); return () => clearTimeout(timer); } }, [selectedTurn]);
  ...
  return <View style={[s.page, { paddingTop: insets.top }]}><ScrollView ref={scroll} keyboardShouldPersistTaps="handled" contentContainerStyle={s.content}>
    ...
    {tab === 0 ? <View onLayout={e => { transcriptOffset.current = e.nativeEvent.layout.y; }}>
      ...
      {turns.map((turn,index) => { const day = new Date(turn.started_at).toLocaleDateString(undefined,{weekday:'long',month:'short',day:'numeric'}); const showDay = index === 0 || new Date(turn.started_at).toDateString() !== new Date(turns[index - 1].started_at).toDateString(); return <View key={turn.id} onLayout={e => {positions.current[turn.id] = e.nativeEvent.layout.y + transcriptOffset.current;}}>
