...
export interface CustomMcp {
  ...
  url: string;
  createdAt: string;
  /** Verified publisher artwork from a future MCP registry adapter. */ logoUrl?: string;
}
...
export function validateMcp(
  ...
  if (url.username || url.password || url.search || url.hash)
    throw new Error(
      "Use a URL without credentials, query parameters or fragments.",
    );
  return { name: label, url: url.toString() };
}
export async function loadCustomMcp(): Promise<CustomMcp[]> {
...
export async function saveCustomMcp(items: CustomMcp[]) {
