...
export interface Approval { id: string; task_id: string; step_id: string; code: string; summary: string; facts: Record<string, string>; args_hash: string; status: 'pending' | 'approved' | 'denied' | 'expired'; channel?: 'sms' | 'app'; expires_at: string; decided_at?: string }
...
export interface Extension { id: string; name: string; description: string; status: 'connected' | 'needs_auth' | 'suggested'; tool_count: number }
