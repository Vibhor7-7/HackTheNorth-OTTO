...
CLAUDE_CODE_DIR              # D-36; default this repository. Where `claude -p` works.
CLAUDE_CODE_BIN              # default claude
CLAUDE_CODE_MAX_TURNS        # default 25
CLAUDE_CODE_TIMEOUT_MS       # default 170000, inside the agent's 3-minute budget
CLAUDE_CODE_ALLOWED_TOOLS    # default Read,Edit,Write,Glob,Grep,Bash
```
...
| AG-13 | P1 | **Claude Code as a tool** (D-36). `CLAUDECODE_RUN { prompt }` runs `claude -p` headless in `CLAUDE_CODE_DIR` and returns its summary. Switched on from the Apps tab like a no-auth toolkit (Connect / Disconnect), shown as a card with the binary version and directory. Pinned R2: the exact prompt is on the approval card. Goes through the gate and the step log like every other tool (AG-3). |
...
| CMP-6 | P1 | Extensions screen data: list Composio toolkits with connection status for the user (connected / needs_auth / suggested) so APP-4 has real data. Names and logos come from the catalogue (D-34); no-auth toolkits the user added are listed as connected. |
...
| ID | Pri | Requirement |
|---|---|---|
| APP-4 | P0 | **Connected tools.** Every Composio toolkit visible to this user (CMP-6): name, icon, status (connected / needs auth / suggested), tool count. **Connect** opens the Connect Link from `POST /api/extensions/:id/connect`. Pull to refresh. Toolkits Otto used recently sort to the top. **Add an app** (D-34) opens a search over `GET /api/catalog`; a managed-OAuth toolkit connects from the row, a no-auth toolkit is added on tap, and a custom-auth toolkit says it needs the dashboard. |
...
GET    /api/catalog?q&limit                  -> CatalogEntry[]               (D-34; Composio's toolkit catalogue, cached server-side)
POST   /api/extensions/:id/connect           -> { link } | { connected: true } (APP-4; D-34: creates the auth config on demand, `connected` for no-auth toolkits)
...
interface Extension {                       // a Composio toolkit as seen by this user
  ...
  tool_count: number;
  logo_url?: string;                        // Composio toolkit metadata (D-34)
}

interface CatalogEntry {                    // GET /api/catalog (D-34)
  slug: string; name: string; description: string;
  logo_url?: string; tool_count: number; categories: string[];
  auth: "managed" | "none" | "custom";      // managed: connects from the phone; none: no account; custom: dashboard only
}
...
**Web search (D-37).** `COMPOSIO_SEARCH_WEB`, `COMPOSIO_SEARCH_NEWS` and
...
| **D-36** | **Claude Code is a task-agent tool, enabled from the Apps tab.** `claude -p` headless in one fixed directory, pinned R2, result summary into the step log. | "Otto, have Claude fix the failing test" is the natural voice request for a team that already lives in Claude Code, and the agent already has the shape for it: discovery, gate, approval card, step log. Verified against Claude Code 2.1.278: `-p --output-format json` returns one result object with `is_error`, `num_turns`, `total_cost_usd`, `permission_denials` and the text in `result`; a nested-session marker in the environment makes it hang, so the child gets a scrubbed env. It is R2 because it edits files and runs commands on the laptop, and the prompt on the card is what the user is approving. The directory is fixed from env (default: this repository) and the agent cannot choose it; that is the whole boundary. Enabled from the app rather than env so the Apps tab is the one place to see what Otto can reach (D-34). Not a Composio tool: it runs on the laptop, so it lives in `local/` and takes a branch in `execute.ts`, which keeps AG-3 true - the gate is still the only path to it. |
| **D-35** | **A task connects the app it needs, and resumes when the user signs in.** Discovery (AG-7) admits any catalogue toolkit the goal names, the gate creates its auth config and raises the Connect Link, and the suspended call retries once on completion. | This closes the `[TODO CMP-4]` that had S0 stopping at the card: the requirement always said "retry the exact same tool call once, then continue", and D-34 made the first half possible from the app. The retry suspends the loop in place, exactly as D-30 does for approvals and for the same reason: the arguments never leave the closure the user saw. Cost, as with D-30: a restart with a connection outstanding strands that task, and a request that is not completed within ten minutes expires and the task ends with a plain step saying so. Catalogue candidates are limited to toolkits the phone can actually connect (managed OAuth, no auth) and to names that appear in the goal, so discovery does not wander. |
...
- **1.9.8**: Otto can search the web (D-37). `composio_search` joins the curated
  ...
  sentence.
- **1.9.7**: Claude Code as a task-agent tool (AG-13, D-36): headless `claude -p`
  in a fixed directory, R2, enabled from the Apps tab, card shows version and
  directory. Five env vars added to 5.2 with working defaults. No Section 7 change.
- **1.9.6**: CMP-4 finished: a task that raised a Connect Link resumes and retries
  ...
  an app is a task. **Section 7 changed** only by that sentence.
- **1.9.5**: **Section 7 changed:** `GET /api/catalog`, `CatalogEntry`, and
  `Extension.logo_url` added; `POST /api/extensions/:id/connect` may answer
  `{ connected: true }` for a no-auth toolkit (D-34). Toolkits are added from
  the app: managed-OAuth auth configs are created on demand, no-auth toolkits
  are enabled locally, custom-auth ones still point at the dashboard. APP-4 and
  CMP-6 updated.
...
- **1.9.1**: The Chat tab stopped answering for services it cannot see. Asked
  ...
  reading. It now states its own limits, an empty read explains what the emptiness
  does and does not mean, and `run_task` is *forced* rather than suggested when the
  question needs live data, because the prompt alone did not hold. **Section 6
  changed:** VG-7 now carries the `source: "voice"` condition it always implied
  (D-31), so a chat-started Task reports on Home per CHAT-4 instead of speaking
  unprompted through the earpiece.
